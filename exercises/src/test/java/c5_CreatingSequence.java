...
public class c5_CreatingSequence {
     ...
     *
     * Answer:
     * - What is difference between `generate` and `create`? - generate is sync, but the create is Async
     * - What is difference between `create` and `push`? - push is able to proceed only with on Thread
     */
    @Test
    public void generate_programmatically() {
        AtomicInteger counterForGenerate = new AtomicInteger(0);
        AtomicInteger counterForCreate = new AtomicInteger(0);
        AtomicInteger counterForPush = new AtomicInteger(0);

        Flux<Integer> generateFlux = Flux.generate(sink -> {
            sink.next(counterForGenerate.incrementAndGet());
            if (counterForGenerate.get() == 5) sink.complete();
