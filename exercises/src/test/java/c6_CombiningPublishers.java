...
public class c6_CombiningPublishers extends CombiningPublishersBase {
     ...
     *
     * Answer:
     * - Is there a difference between Mono.flatMap() and Flux.flatMap()? | No, since flatMap just convert subtype
     */
    @Test
...
//        Flux<Integer> numbers = numberService1().concatWith(numberService2());
     ...
     *   the output will be sequenced in concatMap, but flatMap doesn't guarantee the sequence ordering
     * - What is difference between concatMap() and flatMapSequential()? |
     *  the output will be sequenced in both cases but the concatMap proceed subscriptions sequentially, rather than flatMapSequential which store the result in quenue
     * - Why doesn't Mono have concatMap() operator?
     *  since the Mono present just one element
     */
    @Test
    ...
    public void mail_box_switcher() {
        //todo: feel free to change code as you need
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, primary) -> {
...
//        blizzardTitles();
    ...
    public void car_factory() {
        ...
        Flux<Car> producedCars = carEngineProducer()
                .zipWith(carChassisProducer(), ((engine, chassis) -> {
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(StreamingConnection.startStreaming(),
                Function.identity(),
                clean -> StreamingConnection.closeConnection());
